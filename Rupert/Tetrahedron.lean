...
theorem rupert : IsRupert tetrahedron := by
  ...
  have hb : Metric.ball 0 ε₀ ⊆ convexHull ℝ outer_shadow := by
    ...
    · use ![14470757879961/43300505182000,
      ...
      use fun i ↦ (proj_xy (outer_rot *ᵥ (tetrahedron i)))
      refine ⟨?_, ?_, ?_, ?_⟩
      · intro i; fin_cases i <;> norm_num
      · simp [Fin.sum_univ_four]; norm_num
      · exact fun i ↦ ⟨i, rfl⟩
      · simp only [proj_xy, outer_rot, matrix_of_quat, outer_quat, tetrahedron, Fin.sum_univ_four,
                   ε₀, matrix_simps]
        norm_num
  ...
  · use ![10743981448378145233579223/2255005124571996596714809125,
    ...
    use fun i ↦ (1 - ε₁) • (proj_xy (outer_rot *ᵥ (tetrahedron i)))
    refine ⟨?_, ?_, ?_, ?_⟩
    · intro i; fin_cases i <;> norm_num
    · simp only [Fin.sum_univ_four, matrix_simps]; norm_num
    · exact fun i ↦ ⟨proj_xy (outer_rot *ᵥ tetrahedron i), by simp [outer_shadow]⟩
    · rw [←hy]
      simp only [proj_xy, outer_rot, matrix_of_quat, outer_quat, tetrahedron, Fin.sum_univ_four,
